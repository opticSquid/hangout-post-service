...
public interface HierarchyKeeperRepo extends JpaRepository<HierarchyKeeper, Integer> {
    @Query(value = "SELECT c.comment_id, c.created_at, c.text, c.user_id FROM comment AS c JOIN hierarchy_keeper k ON c.comment_id = k.child_comment_id WHERE k.parent_comment_id = :commentId", nativeQuery = true)
